...
public class WriteRegisterState extends WriteState {
    ...
    public <T> void Write(SlaveWrite<T> writeValue) throws ModbusTransportException, ErrorResponseException {
        ...
        if ("short".equals(simpleName.toLowerCase())) {
            ...
            this.writeService.WriteRegister(master, writeValue.getSlaveId(), writeValue.getOffset(), val);
        }
        //写多个保持寄存器
        else if ("short[]".equals(simpleName.toLowerCase())) {
            short[] val = (short[]) writeValue.getVal();
            ModbusMaster master = MasterFactory.getInstance().GetModbusMaster(writeValue.getMaster());
            this.writeService.WriteRegisters(master, writeValue.getSlaveId(), writeValue.getOffset(), val);
        } else {
