...
public class ReadCoilsStrategy extends StrategyBase {
    ...
    public void Read(int slaveId, List<SlavePoint> points) {
        if (points != null) {
            ...
            boolean[] vals = ReadSlaveByAllPoints(slaveId, points);
     ...
     * @return
     */
    private boolean[] ReadSlaveByAllPoints(int slaveId, List<SlavePoint> points) {
        boolean[] vals = new boolean[points.size()];
        for (int i = 0; i < points.size(); i++) {
            vals[i] = this.readCoilStatus(slaveId, points.get(i).getOffset());
        }
        return vals;
    }
}
